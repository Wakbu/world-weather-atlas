...
- 실시간·캐시 여부, API 제공 상태, 공식 특보 연동 범위를 보여주는 데이터 신뢰도
- 위치별 비·눈·폭염·미세먼지 알림 조건 저장과 지원 브라우저의 주기적 백그라운드 확인
...
- 비구름 레이더, 8×6 표본을 양선형 보간한 구름량·PM2.5 연속 래스터, 고대비 풍향·풍속 표시를 전환하는 기상 지도
...
- 섭씨/화씨 전환
- 라이트/다크 모드 및 설정 저장
- 여백과 정보 위계를 재정리한 친밀한 대시보드, 모바일 전용 하단 내비게이션, 접을 수 있는 위치 상세 정보
...
| 미국 공식 기상 특보 | National Weather Service API |
| 지도 타일 | OpenStreetMap |
...
- 알림은 앱이 열려 있을 때 확실히 검사합니다. Periodic Background Sync 지원 설치형 브라우저에서는 닫힌 상태 확인도 시도하지만 실행 주기는 운영체제와 브라우저가 결정합니다.
...
- 공식 특보는 현재 미국 NWS 지원 지역만 연동합니다. 다른 지역은 Open-Meteo 기반 위험 신호와 공식 특보 미연동 상태를 구분해 표시합니다.
- 구름량과 PM2.5 지도는 48개 모델 표본을 양선형 보간한 시각화이며 관측 레이더 타일이 아닙니다.
